...
exports.logout = (req, res) => {
  res.cookie('jwt', '', {
    expires: new Date(Date.now()), //expires in 10s
    httpOnly: true
  });
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('There is no user with the following email address!', 404));
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  const hashedToken = await crypto
    .createHash('sha256')
    .update(req.params.token) //getting the token from url
    .digest('hex');
